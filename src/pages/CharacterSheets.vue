...
export default {
  ...
  computed: {
    ...
    sheets() {
      return Object.values(this.sheetsByUrl).reduce(
        (acc, sheets) => acc.concat(sheets),
        []
      );
    },
    playerSheets() {
  ...
  methods: {
    checkVersion() {
      return fetch(
        ...
        .then((text) => {
          ...
          let test = doc
            .querySelector("a[href*='/shibuyasgame/characters/commit/'].btn")
            .text.trim();
          return test;
        })
        .catch(() => false);
    ...
    loadSheets(url, index) {
      processSpreadsheet(url)
        ...
        .then(() =>
          this.$nextTick(() => this.$set(this.sheetStatuses, index, true))
        );
    },
    loadData() {
      this.checkVersion()
